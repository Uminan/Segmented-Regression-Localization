...
class RadioUNet_c(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.simulation != "rand":
            ...
            img_name_gainIRT2 = os.path.join(self.dir_gainIRT2, name2) 
            w = np.random.uniform(0, self.IRT2maxW)  # IRT2 weight of random average
            image_gain = w * np.expand_dims(np.asarray(io.imread(img_name_gainIRT2)), axis=2)  \
                         + (1 - w) * np.expand_dims(np.asarray(io.imread(img_name_gainDPM)), axis=2)
        ...
        # Convert image_gain to uint8 format
        image_gain = (image_gain).astype(np.uint8)
...
class RadioUNet_s(Dataset):
    ...
    def __init__(self,maps_inds=np.zeros(1), phase="train",
                 ind1=0,ind2=0, 
                 dir_dataset="/home/weiming/llama/Dataprocess/RadioMapSeer/",
                 numTx=80,                  
                 thresh=0.2,
                 simulation="DPM",
                 carsSimul="no",
